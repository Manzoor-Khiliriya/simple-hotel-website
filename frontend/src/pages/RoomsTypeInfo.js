...
import { CircularProgress, Typography, Card, CardContent, Box, Alert } from '@mui/material';
...
import HeadingTypography from '../components/customHeading/HeadingTypography.js.js';
...
export default function RoomsTypeInfo() {
  ...
  return (
    <Box display='flex' flexDirection='column' justifyContent='center' minHeight='50vh' sx={{ padding: 3, backgroundColor: "#DFAA5B" }}>
        ...
        {isLoading ? (
          ...
          rooms.map((room) => (
            <Grid size={{ xs: 12, sm: 6, md: 4 }} key={room._id}>
              <Link to={`/room-details/${room._id}`} style={{
                textDecoration: "none",
                flex: 1
              }}>
